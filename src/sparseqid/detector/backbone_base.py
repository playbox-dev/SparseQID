...
def get_global_rank() -> int:
    """Global rank of this process, 0 when not running distributed."""
...
class BackboneBase(nn.Module, metaclass=BackboneMeta):
    ...
    def __init__(
        ...
        if isinstance(freeze_at, str):
            if freeze_at != "all":
                raise ValueError(
                    f"If freeze_at is a string, it should be 'all'. Received: {freeze_at}."
                )
        else:
            ...
            if not all(isinstance(i, (int, str)) for i in freeze_at):
                raise ValueError(
                    f"Invalid freeze_at value: {freeze_at}. It should be a list of integers or strings."
                )
        ...
        if self.freeze_at == "all":
            ...
            for key in self.freeze_at:
                if key in stage_dict:
                    ...
                    if get_global_rank() == 0:
                        logging.warning(
                            f"Stage {key} not found. Freezing options: {stage_dict.keys()}"
                        )
