...
def _get_norm_abbr(layer_class: Type[nn.Module]) -> str:
    ...
    if not inspect.isclass(layer_class):
        raise TypeError(f"layer_class must be a type, but got {type(layer_class)}")
...
def build_backbone(backbone_type, **kwargs):
    """Build a Sparse4D image backbone by config type name."""
    if backbone_type not in _BACKBONES:
        raise KeyError(f"unknown backbone type {backbone_type!r}; available: {sorted(_BACKBONES)}")
    return _BACKBONES[backbone_type](**kwargs)
