...
from .backbone import build_norm_layer
...
def build_padding_layer(cfg: Dict[str, Any], *args, **kwargs) -> nn.Module:
    ...
    if layer_type == "zeros":
        ...
        raise ValueError(
            "Padding mode 'zeros' is handled by the convolution layer, not as a separate padding layer."
        )
...
def kaiming_init(module, a=0, mode="fan_out", nonlinearity="relu", bias=0, distribution="normal"):
    ...
    assert distribution in ["uniform", "normal"], (
        f"distribution must be one of 'uniform', 'normal', but got {distribution}"
    )
    if hasattr(module, "weight") and module.weight is not None:
...
class ConvModule(nn.Module):
    ...
    def __init__(
        ...
        assert conv_cfg is None or isinstance(conv_cfg, dict), (
            f"conv_cfg must be a dict, but got {type(conv_cfg)}"
        )
        assert norm_cfg is None or isinstance(norm_cfg, dict), (
            f"norm_cfg must be a dict, but got {type(norm_cfg)}"
        )
        assert act_cfg is None or isinstance(act_cfg, dict), (
            f"act_cfg must be a dict, but got {type(act_cfg)}"
        )
        ...
        assert isinstance(self.order, tuple) and len(self.order) == 3, (
            f"order must be a tuple of length 3, but got {self.order}"
        )
        assert set(order) == {"conv", "norm", "act"}, (
            f"order must contain 'conv', 'norm', 'act', but got {set(order)}"
        )
...
class FPN(nn.Module):
    ...
    def __init__(
        ...
        if end_level == -1 or end_level == self.num_ins - 1:
            ...
            assert num_outs >= self.num_ins - start_level, (
                f"num_outs must be >= self.num_ins - start_level, but got {num_outs} >= {self.num_ins - start_level}"
            )
        else:
            # if end_level is not the last level, no extra level is allowed
            self.backbone_end_level = end_level + 1
            assert end_level < self.num_ins, (
                f"end_level must be < self.num_ins, but got {end_level} < {self.num_ins}"
            )
            assert num_outs == end_level - start_level + 1, (
                f"num_outs must be equal to end_level - start_level + 1, but got {num_outs} != {end_level - start_level + 1}"
            )
        self.start_level = start_level
        self.end_level = end_level
        self.add_extra_convs = add_extra_convs
        assert isinstance(add_extra_convs, (str, bool)), (
            f"add_extra_convs must be a str or bool, but got {type(add_extra_convs)}"
        )
        if isinstance(add_extra_convs, str):
            # Extra_convs_source choices: 'on_input', 'on_lateral', 'on_output'
            assert add_extra_convs in ("on_input", "on_lateral", "on_output"), (
                f"add_extra_convs must be one of 'on_input', 'on_lateral', 'on_output', but got {add_extra_convs}"
            )
    ...
    def forward(self, inputs):
        ...
        assert len(inputs) == len(self.in_channels), (
            f"len(inputs) must be equal to len(self.in_channels), but got {len(inputs)} != {len(self.in_channels)}"
        )
            ...
            # add conv layers on top of original feature maps (RetinaNet)
                ...
                if self.add_extra_convs == "on_input":
                    ...
                    raise NotImplementedError(
                        f"add_extra_convs must be one of 'on_input', 'on_lateral', 'on_output', but got {self.add_extra_convs}"
                    )
